 ...
 * @author
 */
public class WorkPositionProcessAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (DialogForm.ACTION_ADD.equalsIgnoreCase(action)) {
				processor.addWorkPosition(workPositionDialogForm.getVo());
				workPositionDialogForm.setFlagOperation(true);
			} else if (DialogForm.ACTION_EDIT.equalsIgnoreCase(action)) {
				processor.updateWorkPosition(workPositionDialogForm.getVo());
				workPositionDialogForm.setFlagOperation(true);
			} else if (DialogForm.ACTION_DELETE.equalsIgnoreCase(action)) {
				processor.removeWorkPosition(workPositionDialogForm.getVo().getWorkPosition());
				workPositionDialogForm.setFlagOperation(true);
			}
		} catch (Exception e) {
