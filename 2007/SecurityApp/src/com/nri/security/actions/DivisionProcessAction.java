 ...
 * @author
 */
public class DivisionProcessAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (DialogForm.ACTION_ADD.equalsIgnoreCase(action)) {
				processor.addDivision(divisionDialogForm.getVo());
				divisionDialogForm.setFlagOperation(true);
			} else if (DialogForm.ACTION_EDIT.equalsIgnoreCase(action)) {
				processor.updateDivision(divisionDialogForm.getVo());
				divisionDialogForm.setFlagOperation(true);
			} else if (DialogForm.ACTION_DELETE.equalsIgnoreCase(action)) {
				processor.removeDivision(divisionDialogForm.getVo().getDivision());
				divisionDialogForm.setFlagOperation(true);
			}
		} catch (Exception e) {
		...
		if (!errors.isEmpty()) {
			...
			forward = mapping.findForward(divisionDialogForm.getForwardName());
