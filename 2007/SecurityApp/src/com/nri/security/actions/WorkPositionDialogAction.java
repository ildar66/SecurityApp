...
import com.nri.security.forms.WorkPositionDialogForm;

/**
 * @version 	1.0
 * @author
 */
public class WorkPositionDialogAction extends Action {

	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {

		ActionErrors errors = new ActionErrors();
		ActionForward forward = new ActionForward(); // return value
		WorkPositionDialogForm workPositionDialogForm = (WorkPositionDialogForm) form;

		try {
			// Get actionprocess (model) and delegate
			SecurityActionProcessor processor = (SecurityActionProcessor) ActionProcessorFactory.getActionProcessor("Security");
			String action = workPositionDialogForm.getAction();
			if (DialogForm.ACTION_EDIT.equalsIgnoreCase(action) || DialogForm.ACTION_VIEW.equalsIgnoreCase(action) || DialogForm.ACTION_DELETE.equalsIgnoreCase(action)) {
				workPositionDialogForm.setVo(processor.findWorkPositionByKey(workPositionDialogForm.getVo().getWorkPosition()));
			} else if (DialogForm.ACTION_ADD.equalsIgnoreCase(action)) {
				int id = OIDGeneratorHelper.getNextId("tables.workpositions");
				workPositionDialogForm.setVo(new WorkPosition(new Integer(id)));
				workPositionDialogForm.getVo().setIsActive(Boolean.TRUE);
			}
		} catch (Exception e) {
