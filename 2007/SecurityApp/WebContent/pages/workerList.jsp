...
<%
	java.lang.String gColSpan = "7";
 	WorkerListForm listForm = (WorkerListForm)session.getAttribute("WorkerListForm");
	String sortBy = listForm.getOrderBy();
	java.util.List posList = (java.util.List)request.getAttribute(BeanKeys.WORKER_LIST);
...
<H3 id='bread-crumb'>
	<html:link page="/companyList.do">Компании</html:link> &gt;
		Штат компании <I><bean:write name="WorkerListForm" property="companyVO.name"/></I>  &gt;
</H3>
<BR>
<html:form action="/workerList.do" styleId="listForm">
<html:errors/>
<html:hidden property="orderBy"/>
<html:hidden property="selectedID" value="0"/>
<%@include file="../secure/layouts/tableJS.jspf"%>

<html:hidden property="query.company"/>
		...
		<tr class="layout-manager">
			...
			<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="1">
				<TR>
					<TD class="find-filter">
						<% if(canEdit) { %>
							<html:link page="/workerDialog.do?action=Add&forwardName=workerList" paramId="company" paramName="WorkerListForm" paramProperty="query.company">
...
<% } %>
        ...
        <!-- Table body -->
        <logic:iterate id="worker" name="<%=BeanKeys.WORKER_LIST%>" type="com.nri.custom.WorkerTO"> 
            ...
            <tr id='<bean:write name="worker" property="worker" filter="true"/>' class="normal" onMouseOver="className='select'" onMouseOut="onMouseOutRow(this.id);" onclick="onClickRow(this.id);">
            	<td class="text">
                    <html:link page="/workerDialog.do?action=Edit&forwardName=workerList" paramId="worker" paramName="worker" paramProperty="worker" paramScope="page">            	
...
<% if(canEdit) { %>
                <td class="text">
					...
					<html:link page="/workerDialog.do?action=Delete&forwardName=workerList" paramId="worker"
