...
<%
	...
	WorkPositionLookupListForm listForm = (WorkPositionLookupListForm)session.getAttribute("WorkPositionLookupListForm");
	String sortBy = listForm.getOrderBy();
	java.util.List posList = (java.util.List)request.getAttribute(BeanKeys.WORK_POSITION_LOOKUP_LIST);
	int posListSize = posList.size();//кол-во позиций.	
%>

<html:form action="/workPositionLookupList.do" styleId="listForm">
<html:errors/>
...
<july:separator colspan="<%= gColSpan %>" />
        ...
        <!-- Table body -->
    <logic:iterate id="wpl" name="<%=BeanKeys.WORK_POSITION_LOOKUP_LIST%>" type="com.nri.domain.WorkPosition">
        <logic:present name="wpl">
            <tr id='<bean:write name="wpl" property="workPosition" filter="true"/>' class="normal" onMouseOver="className='select'" onMouseOut="onMouseOutRow(this.id);" onclick="onClickRow(this.id);">
                ...
                <td class="text">
					<A href='javascript:Go("<bean:write name="wpl" property="workPosition" filter="true"/>|<bean:write name="wpl" property="name" filter="true"/>")' title="Выберите запись">                
						<bean:write name="wpl" property="name" filter="true"/></A>
                </td>
                <td class="text">
						<%--html:link 
